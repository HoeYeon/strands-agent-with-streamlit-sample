...
- [ ] 3. 스키마 정보 추출 및 구조화
- [ ] 3.1 Markdown 파싱 및 정보 추출
  - 테이블명, 데이터베이스명 추출
  - 컬럼 정보 파싱 (이름, 타입, 별칭, 설명)
  - 비즈니스 로직 섹션 추출
  - SchemaInfo, ColumnDetail 데이터 모델 생성
  - _Requirements: 1.5_

- [ ]* 3.2 스키마 정보 구조화 property 테스트 작성
...
- [ ] 3.3 도메인 지식 추출 구현
  - 비즈니스 용어 매핑 정보 추출
  - 메트릭 정의 및 계산 공식 추출
  - DomainMapping 데이터 모델 생성
  - _Requirements: 2.1, 2.2, 2.3_

- [ ]* 3.4 도메인 지식 검색 property 테스트 작성
  - **Property 6: 도메인 지식 검색**
  - **Validates: Requirements 2.1**

- [ ]* 3.5 메트릭 정의 정보 포함 property 테스트 작성
  - **Property 7: 메트릭 정의 정보 포함**
  - **Validates: Requirements 2.2**

- [ ]* 3.6 용어 매핑 제공 property 테스트 작성
  - **Property 8: 용어 매핑 제공**
...
- [ ]* 3.8 출처 및 신뢰도 포함 property 테스트 작성
  - **Property 9: 출처 및 신뢰도 포함**
  - **Validates: Requirements 2.5**

- [ ] 4. 공유 컨텍스트 통합
- [ ] 4.1 AnalysisContext 확장
  - rag_schema_results 필드 추가
  - rag_domain_results 필드 추가
  - rag_enabled 플래그 추가
  - 검색 결과 저장 및 조회 메서드 구현
  - _Requirements: 3.4_

- [ ]* 4.2 공유 컨텍스트 저장 property 테스트 작성
  - **Property 13: 공유 컨텍스트 저장**
  - **Validates: Requirements 3.4**

- [ ] 5. 기존 에이전트와의 통합
- [ ] 5.1 Data Expert Agent 수정
...
- [ ] 6. 첫 번째 체크포인트 - 모든 테스트 통과 확인
  - 모든 테스트가 통과하는지 확인하고, 문제가 있으면 사용자에게 질문

- [ ] 7. 에러 처리 및 복구 로직 구현
- [ ] 7.1 OpenSearch 연결 에러 처리
...
- [ ] 8. 통합 테스트 및 검증
- [ ] 8.1 End-to-End 워크플로우 테스트
  - 사용자 쿼리 → RAG 검색 → 테이블 매칭 → SQL 생성
  - RAG 없이 동작 확인
  - RAG 실패 시 복구 확인
  - _Requirements: 3.1, 3.2, 3.3, 3.4, 3.5_

- [ ] 8.2 OpenSearch 통합 테스트
...
- [ ] 9. 문서화 및 설정 가이드 작성
- [ ] 9.1 OpenSearch 설정 가이드 작성
  - 인덱스 생성 방법
  - 벡터 필드 설정
  - 접근 권한 설정
  - _Requirements: 3.1_

- [ ] 9.2 사용 예제 및 코드 샘플 작성
  - RAG Agent 초기화 예제
  - 검색 쿼리 예제
  - 에러 처리 예제
  - _Requirements: 1.1, 1.2, 1.3_

- [ ] 10. 최종 체크포인트 - 모든 테스트 통과 확인
  - 모든 테스트가 통과하는지 확인하고, 문제가 있으면 사용자에게 질문
