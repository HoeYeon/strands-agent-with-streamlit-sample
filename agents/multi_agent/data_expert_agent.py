...
**스키마 문서 검색이 필요한 경우**:
- 테이블/컬럼 설명이 더 필요할 때
- 비즈니스 로직이나 값 설명이 필요할 때
- 도메인 지식이 필요할 때

**RAG Agent 호출 방법**:
handoff_to_agent(
    agent_name="rag_agent",
    message="스키마 검색 요청: {사용자 쿼리}"
)

**RAG 검색 결과 활용**:
- 검색된 스키마 문서를 테이블 매칭에 활용
- 비즈니스 로직을 sql_agent에 전달
...
1. (선택) rag_agent로 스키마 문서 검색
2. list_databases
3. list_tables (max_results=50)
4. lead_agent로 handoff
...
2. list_databases
3. list_tables (max_results=50)
4. get_table → 컬럼명 + 타입 상세 조회
5. sql_agent로 handoff (아래 형식 필수)
